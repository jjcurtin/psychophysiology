 function mHealth_NPU_Predictable
%mHealth NPU Predictable Shock Block
%JTK & KM
...
try
    ...
    CueShockTime(1,:)=[LCS LCS LCS LCS LCS LCS];
    ...
    RootPath = fileparts(which('mHealth_NPU_Predictable.m'));
    FileType = 'JPG';
    
    %Cues stored in cell array (CueImg) or (ITIImg as question (1=no question) X CueType (1)
    CueImg = cell(1,1);
    CueImg{1,1} =  imread([RootPath '/mHealth/NPU/CueP.JPG'], FileType);
    ...
    %ITI image
    ITIImg = cell(1,1);
    ITIImg{1,1} =  imread([RootPath '/mHealth/NPU/ITIP.JPG'], FileType);
